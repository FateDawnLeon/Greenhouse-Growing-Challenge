...
from utils import load_json_data, normalize
...
class GreenhouseClimateDataset(Dataset):
    EP_KEYS = [
        'common.Iglob.Value',
        'common.TOut.Value',
        'common.RHOut.Value',
...
class GreenhousePlantDataset(Dataset):
    OP_IN_KEYS = [
        "comp1.Air.T",
        "comp1.Air.RH",
        "comp1.Air.ppm",
    ...
    def get_norm_data(norm_data_dirs):
        ...
        for data_dir in norm_data_dirs:
            for name in os.listdir(f"{data_dir}/outputs"):
                output = load_json_data(f"{data_dir}/outputs/{name}")
                op_in = parse_output(output, GreenhousePlantDataset.OP_IN_KEYS)
                op_pl = parse_output(output, GreenhousePlantDataset.OP_PL_KEYS)
                op_in_list.append(op_in)
